...
$qry = $conn->query("SELECT * FROM `client_tb` where id = '{$_settings->userdata('id')}'");
if($qry->num_rows > 0){
...
<body class="index-page bg-gray-200">
    ...
    <div class="row justify-content-center mx-0">
        <div class="col-lg-8 col-md-10 col-sm-12 col-xs-12">
            <div class="card card-body shadow-blur mx-3 mx-md-4 rounded-0">
                ...
                <?php endif; ?>
                <div class="container">
                    <h4 class="fw-bolder text-center">Client Registration</h4>
                    <hr>
                    <br>
                    <form action="Clients.php" id="register-user-form">
    ...
    start_loader()
    $.ajax({
        url:'../classes/Clients.php?f=save_client',
        ...
        error:err=>{
            console.error(err)
            el.text("An error occurred while saving data1")
            _this.prepend(el)
            el.show('slow')
            $('html, body').scrollTop(_this.offset().top - '150')
            end_loader()
        ...
        success:function(resp){
            if(resp.status == 'success'){
                location.href = "./Client/client_login.php";
            }else if(!!resp.msg){
                el.text(resp.msg)
                _this.prepend(el)
                el.show('slow')
                ...
                el.text("An error occurred while saving data2")
                _this.prepend(el)
                el.show('slow')
                $('html, body').scrollTop(_this.offset().top - '150')
            }
